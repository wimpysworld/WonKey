...
**Public `key COMBINATION` and `rgb MODE [RGB]` commands can upload and commit settings after confirmation.** Bare `key` and `rgb` only read settings.
The separate developer binary retains legacy `apply --write`. There is no arbitrary packet sender, firmware writer, reset, or bootloader command.
...
Run command examples from the project root after [building wonkey-dev separately](development.md#separate-build).
...
`wonkey-dev protocol preview` remains offline and deliberately constructs a complete replacement. It is not an apply input.
...
```sh
./wonkey-dev protocol preview --replace-all --key enter --modifiers 0 --trigger 1 \
  --rgb-mode 1 --red 0 --green 0 --blue 255
./wonkey-dev protocol parse-identify --hex "$IDENTIFY_HEX"
./wonkey-dev protocol parse-readback --identify "$IDENTIFY_HEX" \
...
See [lighting](usage.md#lighting) for the full vendor mode table and [configuration fields](usage.md#supported-configuration-fields) for writable offsets.

The top-level `preview`, `parse-identify`, and `parse-readback` aliases remain available in `wonkey-dev` only.
